...
bot.on('message', msg=>{
  if (msg.content.startsWith(prefix)) {
    ...
    if (argTest(3)) {
      for (var i = 0; i < 3; i++) {
        if (!isNaN(Number(msgArg[i])) && ((4 <= Number(msgArg[i]) && Number(msgArg[i]) <= 16) || (i == 2 && 0 <= Number(msgArg[i])))) {
    ...
    if (argDone == 3) {
      ...
      for (var i = 0; i < msgArg[1]; i++) {
        for (var j = 0; j < msgArg[0]; j++) {
          if (fieldArr[i][j] != 11) {
            nearMine = 0;
            for (var k = 0; k < 3; k++) {
              if ((k == 0 && 0 < i) || k == 1 || (k == 2 && i < msgArg[1]-1)) {
                if (0 < j) {
      ...
      msg.channel.send('**Size: ' + msgArg[0] + 'x' + msgArg[1] + '**\n**Mine Count: ' + msgArg[2] + '**\n**Have fun :D**');
    } else {
      msgToSend = new Discord.MessageEmbed()
      .setColor('#121212')
      .setAuthor('How to use', '')
      .setDescription('-g {width(min:4,max:16)} {height(min:4,max:16)} {mine}');
      msg.channel.send(msgToSend);
    }
  }
})
